...
class Grygry:
    ...
    def grep_pattern(self):
        if not self.pattern:
            raise NoPatternError
        self.grep_folder(Path.cwd())

    def grep_folder(self, path: Path):
        self.path = path
    ...
    def _list_files(self, path: Path, files: list, sub_dirs: list):
        if path.is_dir():
    ...
    def _show_found_words(self, files: list):
        for path in files:
            self.show_find_word(path)

    def _sorted_show_found_words(self, files: list):
        for path in sorted(files):
            self.show_find_word(path)

    def show_find_word(self, path: Path):
        self.find_word(path)
        if not self.found:
            return
        self.show_found(path)

    def show_found(self, path: Path):
        print(path)
    ...
    def _show_found_context(self, sorted_dict):
        previous = 0
    ...
    def _show_found_no_context(self, sorted_dict: dict[int, str]):
        for idx, line in sorted_dict.items():
            print(f" {idx:4d}: {line.rstrip()}")
